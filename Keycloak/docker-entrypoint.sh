...
##########################
# create config-bkat1.json
##########################
#cat /opt/jboss/keycloak/config-bkat1.template | sed -e "s|{{PORTAL_USER_PASSWORD}}|${PORTAL_USER_PASSWORD}|g" -e "s|{{BKAT1_USER_PASSWORD}}|${BKAT1_USER_PASSWORD}|g" > /opt/jboss/keycloak/config-bkat1.json
cat /opt/jboss/keycloak/config.template > /opt/jboss/keycloak/config.json

##################
# Start Keycloak #
##################
exec /opt/jboss/keycloak/bin/standalone.sh $@ -b 0.0.0.0 -Dkeycloak.import=/opt/jboss/keycloak/config.json
exit $?
